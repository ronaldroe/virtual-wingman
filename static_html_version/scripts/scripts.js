function PageSetup(){
    ...
    this.init = function(input_data){
        ...
        $.get('../units.json', function(data){
            ...
            // Check if we're passing data or looking for it.
            login_data = typeof input_data == 'object' ? match_login(input_data, db) : _this.find_login(data, db);
            
            // If we have login data and it's correct, log in.
            if(login_data){
                _this.log_in(login_data);
            } else {
                // If the data is incorrect and we're not passing data, the password was incorrect.
    ...
    // Check for and add custom numbers - PRIVATE
...
window.onload = function(){
    ...
    $('section.login form').submit(function(evt){
        ...
        // User tried to log in. Try to init the page
        page.init({unit: document.forms[2][0].value, pass: document.forms[2][1].value});
        
        // Hide the login form and show the buttons
        var form_section = document.querySelector('section.login');
...
// Functionalities not related to page setup
...
// Return an object with viewport information
function get_viewport(){
    var viewport = {
        w: $(window).outerWidth(true),
        h: $(window).outerHeight(true),
        o: this.w < this.h ? 'portrait' : 'landscape'
    };
...
// Log out (clear localStorage and refresh page)
function log_out(){
